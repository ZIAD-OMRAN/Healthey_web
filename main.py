...
def _find_frontend() -> Path:
    ...
    Checks multiple candidate locations so the server works regardless
    of which directory it is launched from.
    """
    candidates = [
        ROOT / "frontend",                         # project_root/frontend/  (normal)
        Path(__file__).parent.parent / "frontend", # same as ROOT, explicit
        Path.cwd() / "frontend",                   # wherever the shell is
        Path(__file__).parent / "frontend",        # inside api/ (fallback)
    ]
    for candidate in candidates:
        if (candidate / "index.html").exists():
            return candidate.resolve()
    # Nothing found - return ROOT/frontend so errors are clear
...
# Frontend routes
#
# index.html uses bare relative paths:  href="style.css"  src="app.js"
# When served from "/", the browser requests "/style.css" and "/app.js".
#
# We register explicit FileResponse routes for every frontend asset.
# This keeps all /health, /docs, /nutrition/... API routes fully intact
# (a root-level StaticFiles mount would swallow them).
# ---------------------------------------------------------------------------
...
def serve_index():
    ...
    if not f.exists():
        raise HTTPException(
            404,
            f"index.html not found. Looked in: {FRONTEND_DIR}",
        )
...
# /static mount for any additional assets added in the future
...
# ---------------------------------------------------------------------------
# Startup (lifespan-compatible, deprecation-free)
# ---------------------------------------------------------------------------
...
async def lifespan(application: FastAPI):
    ...
    logger.info("Frontend exists:    %s", FRONTEND_DIR.exists())
    try:
        get_nutrition()
        get_meal_gen()
        get_feedback()
...
# Re-create app with lifespan to silence the on_event deprecation warning
